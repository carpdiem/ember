...
const rawCase=new URLSearchParams(location.search).get('case')??'0';
const numericCase=Number(rawCase); const caseValid=Number.isFinite(numericCase);
const index=caseValid?Math.max(0,Math.min(product.length-1,Math.floor(numericCase))):null;
const row=caseValid?product[index]:null; const style=caseValid?spec.contract.styles[row.style]:null;
...
const dash=caseValid&&style.dasharray?`stroke-dasharray="${{style.dasharray.join(' ')}}"`:'';
...
if(!caseValid){{document.getElementById('status').textContent=`ERROR · invalid ?case=${{rawCase}} · expected a finite number`;document.getElementById('probe').innerHTML='<strong data-status="ERROR">ERROR: invalid case selector</strong>';}}
else{{const filter=row.state==='transformed'?' filter="url(#g1-warm)"':'';
...
def build_core_outputs(output_dir: Path = HERE) -> None:
    ...
    # Browser evidence is provenance-bound captured output. Never overwrite it from the
    # deterministic core generator; g1_browser_validate.py is its sole writer.
    artifacts = {
