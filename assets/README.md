...
The four `mona-lisa-<palette>.png` files resize the source to 640 pixels wide, map pixels by Oklab perceptual lightness, stretch the 1st–99th percentile range, and map it into each family's 256-sample sequential colors. Generation orders any authored ramp from darkest to lightest for this photographic mapping so dark source pixels remain dark and light source pixels remain light. The 3400 K Dark and 3400 K Light derivatives are byte-identical because both interface modes share one canonical 3400 K data ramp.
