...
function sync(){{try{{const u=new URL(location.href);u.searchParams.set('profile',body.dataset.profile);u.searchParams.set('state',body.dataset.state);u.searchParams.set('candidate',body.dataset.focus);history.replaceState(null,'',u)}}catch(e){{/* file:// origins may reject replaceState; controls still work */}}}}
